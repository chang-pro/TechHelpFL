...
export const PlaceCard = ({
  ...
  return (
    <motion.div
      ...
      // --- END NEW ---
      className={cn(
        'w-full max-w-sm overflow-hidden rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-50 shadow-lg hover:border-orange-300 dark:hover:border-orange-600 transition-all duration-200 cursor-pointer',
        className
      )}
    >
      ...
      <div className="relative group h-64">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          {tags.map((tag) => (
            <Badge key={tag} variant="secondary" className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm">
              {tag}
            </Badge>
          ))}
        </div>
        <div className="absolute top-3 right-3">
          <Badge variant="secondary" className="flex items-center gap-1 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm">
